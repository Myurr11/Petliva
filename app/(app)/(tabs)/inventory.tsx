      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        {record.foods.length === 0 ? (
          ...
          record.foods.map((food) => (
            <FoodStockCard key={food.id} petId={activePetId} food={food} restocks={record.restocks.filter((r) => r.foodId === food.id)} />
          ))
        )}
      </ScrollView>
    </SafeAreaView>
...
function FoodStockCard({ petId, food, restocks }: { petId: string; food: FoodItem; restocks: StockEntry[] }) {
...
const styles = StyleSheet.create({
  ...
  foodTitle: { fontFamily: fonts.bodySemibold, fontSize: 15, color: colors.ink, flexShrink: 1 },
